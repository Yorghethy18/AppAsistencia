<?php
  ...
  if (isset($_POST['fecha_inicio']) && isset($_POST['fecha_fin'])) {
    ...
    $sql = "CALL sp_reporte_hora_extras_semanal('$fecha_inicio', '$fecha_fin')";
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while($row = mysqli_fetch_assoc($result)) {
        echo "<tr>
                <td align = 'center' >".$row["TRABAJADOR"]."</td>
                <td align = 'center' >".$row["SEMANA"]."</td>
                <td align = 'center' >".$row["TOTAL_HORA_EXTRA"]."</td>
              </tr>";
      }
      echo "</table>";
    } else {
      echo "<script>alert('No se encontraron resultados en el rango de fechas seleccionado'); window.location.href = '../PHP/horas_semanal.php';</script>"; 
    }
  }
